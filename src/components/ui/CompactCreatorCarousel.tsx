...
import { ChevronLeft, ChevronRight, Heart, Zap, Star, TrendingUp } from 'lucide-react'
...
function CreatorAvatar({ creator, size = 40 }: { creator: Creator; size?: number }) {
    const [imageError, setImageError] = useState(false)

    const avatarUrl = !imageError && creator.avatarUrl
        ? creator.avatarUrl
...
function StatsBadge({ icon: Icon, value, label, color }: {
    icon: any;
    value: string | number;
    label: string;
    color: string;
...
export function CompactCreatorCarousel({
    ...
    const prevSlide = () => {
        setCurrentIndex((prevIndex) =>
            prevIndex === 0 ? Math.max(creators.length - maxVisible, 0) : prevIndex - 1
        )
    }
    ...
    const visibleCreators = creators.slice(currentIndex, currentIndex + maxVisible)
                ...
                {creators.length > maxVisible && (
                    <div className="flex items-center space-x-3">
                        <div className="flex space-x-1">
                            {Array.from({ length: Math.ceil(creators.length / maxVisible) }).map((_, index) => (
                                <button
                                    key={index}
                                ...
                                <div className="relative z-10">
                                    ...
                                    <div className="flex items-center justify-between pt-3 border-t border-gray-200/50 dark:border-gray-600/50">
                                        <div className="flex items-center space-x-3">
                                            ...
                                            {creator.totalAmountReceived && BigInt(creator.totalAmountReceived) > 0 && (
                                                <StatsBadge
                                                    icon={TrendingUp}
                                        ...
                                        {/* Status Indicator */}
                                        <div className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${creator.tipCount > 10
                                        ...
                                        <div className="mt-2">
                                            ...
                                            <div className="h-1 bg-gray-200 dark:bg-gray-600 rounded-full overflow-hidden">
                                                <div
                                                    ...
                                                    style={{ width: `${Math.min(creator.tipCount * 10, 100)}%` }}
                                                />
                                            </div>
                                        </div>
                                    )}
